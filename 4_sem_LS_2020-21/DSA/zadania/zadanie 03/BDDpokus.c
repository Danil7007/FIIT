...
BF *vytvorFunkciu (int min) {
    BF *funkcia = (struct binaryFunction*) malloc (sizeof (struct binaryFunction));
    int mocnina = (rand() % (13 - min + 1)) + min; // viac ako 13 je celkom vela uz
    int dlzka = 2;
    char *vektor = (char*)calloc(32768, sizeof (char)); // 2 na 15 myslim
...
void traverse(node* uzol, BDD *bdd) {
    ...
    if (strlen(vektor) % 2 == 0) {
        ...
        if (jeZbytocny(uzol->funkcia.vektor) == 1) {
            ...
            uzol->false = NULL;
            uzol->true = NULL;
            // todo: nebolo by odveci uvolnit uzly co uz netreba
        }
...
int velkyTest() {
    ...
    int pocetHlavnychTestov = 10;
    int pocetPodtestov = 5;
    int min = 1;
    ...
    for (int j = 0; j < pocetHlavnychTestov; j++) {
        ...
        // root funkcia
        BF *funkcia = vytvorFunkciu(min); // velkost stromu zavisi od vytvorenej funkcie
...
void generateAllBinaryStrings(int n, char arr[], int i, BDD* diagram)
{
    if (i == n) {
        // use
        char vysledok = BDD_use(diagram, arr);
        printf("Vysledna boolean hodnota je: %c\n", vysledok);
        return;
    }

    arr[i] = '0';
    generateAllBinaryStrings(n, arr, i + 1, diagram);
...
int megaVelkyTest() {
    ...
    int n = 13; // pocet premennych
    char arr[20];

    BF *funkcia = vytvorFunkciu(13); // velkost stromu zavisi od vytvorenej funkcie
    BDD *diagram = BDD_create(funkcia);
    printf("Pocet outputov funkcie: %d\n", strlen(funkcia->vektor));
    printf("Pocet premennych pre diagram: %d\n\n", diagram->pocetPremennych);
    ...
    generateAllBinaryStrings(n, arr, 0, diagram);
