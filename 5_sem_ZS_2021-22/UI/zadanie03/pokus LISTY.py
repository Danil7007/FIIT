...
class glob:
    ...
    ZASEKNUTIE = 0  # flag ci moze ukoncit hrabanie v zahradke ( 1 moze / inak nemoze )

    LIMIT = 95000

    POCET_MNICHOV = 20           # pre (dalsie generacie) mnichov
    POCET_MNICHOV_PRVA_GEN = POCET_MNICHOV
    POCET_MAX_MNICHOV = 1
...
def vytvorZahradu(source):
    ...
    try:
        for i in subor:
            ...
            if i.isdigit() and listy_flag == 0:
            ...
            elif not i.isdigit():    # ked narazi na predelovac
                listy_flag = 1
                #print(i)
                ...
                if poradie % 3 == 2:
                    ...
                    pocetListov += 1
                    # print("list:", pocetListov, "x:", listX, "y:", listY)
                    cisloListu = int(i) * -1
                    zahrada.policka[listX][listY] = cisloListu
    ...
    print("\nZahrada vytvorena, rozmer", zahrada.zahradaX, "x", zahrada.zahradaY, "\npocet kamenov", pocetKamenov, "pocet listov", pocetListov, "pocet policok", zahrada.pocetPolicok, "obvod", zahrada.obvod)
    ...
    for x in range(0, zahrada.zahradaX):
        for y in range(0, zahrada.zahradaY):
            ...
            if policko == "K":
                ...
                print(farby.LIST, "", policko*(-1), farby.ENDC, end='')
            else:
                if int(policko) < 10:
                    if int(policko) == 0:
                    ...
                    while (aktX >= 0 and aktX < zahrada.zahradaX) and (aktY >= 0 and aktY < zahrada.zahradaY):
                        ...
                        # ak to NIE JE volne policko -------------------------------------------------
                            ...
                            # ak este VIE ZMENIT SMER
                                ...
                                # ak sme narazili na LIST
                                elif (kopiaZahrady[aktX][aktY] < 0):
                                    # ak je to spravny list v poradi
                                    if kopiaZahrady[aktX][aktY] >= mnich[glob.LIST]:
                                        kopiaZahrady[aktX][aktY] = cisloCesty  # pohrabeme
                                        mnich[glob.POC] += 1  # pripocitame
                                        zaciatky.append(poz)  # pridame kde sme ozaj boli
                                        smery.append(mnich[glob.OPER][j])  # smer ktorym sme naozaj isli
                                        ...
                                        list_typ = int(kopiaZahrady[aktX][aktY]) * (-1)     # odcitame od poctu listu toho typu
                                        zahrada.pocetListov[list_typ] -= 1

                                        if zahrada.pocetListov[list_typ] == 0:      # mnich moze zacat hladat dalsi druh
                                            mnich[glob.LIST] -= 1

                                    # ak je to list mimo poradia
                                    elif kopiaZahrady[aktX][aktY] < mnich[glob.LIST]:
                                        aktX -= posunX  # posunie sa naspat pred list
                                        aktY -= posunY
                                        kopiaZahrady[aktX][aktY] = 0  # nastavi sa to na nepohrabane
                                        mnich[glob.POC] -= 1  # cize ho ani nepocita
                                        # print("vraciam sa na", aktX, aktY)
                                        break  # dalsi cyklus - skusi dalsi smer z policka pred listom
...
def sparuj(rodic1, rodic2):
    ...
    potomok = [new_start, new_oper, 0, 0, -1]
    return potomok
...
if __name__ == "__main__":
    ...
    while pokracuj:
        ...
        print("\t-> pre vyber mapy a spustenie programu stlac \t\t\t(a)\n"
              ...
              "\t-> pre riesenie zahradky s listami stlac \t\t\t\t(d)\n"
              "\n\t-> pre zmenu limitu maximalneho poctu generacii stlac \t(e) ")
        swtch = input()
        ...
        if swtch == "a":
            ...
            print("\nRiesenie zahrady zo zadania:")
            glob.ZASEKNUTIE = 1
            glob.VYBER = 2
            vyries("zahrada1.txt")

        elif swtch == "d":
            print("\nRiesenie zahrady s listami zo zadania:")
            glob.ZASEKNUTIE = 1
            glob.VYBER = 2
            vyries("zahrada7.txt")
