...
float uhol (BOD bodA, BOD bodB) {
  ...
  // zelena bodka
  if ((bodB.x > bodA.x) && (bodB.y == bodA.y)) { 
    uhol = 0; // nula stupnov
  }
  else if (deltaX < 0 && deltaY < 0) { // prvy kvadrant
    uhol = atan(deltaY / deltaX);
  }

  // cervena bodka
  else if ((bodB.x == bodA.x) && (bodB.y > bodA.y)) { 
    uhol = 1.5708; // 90 stupnov
  }
  else if (deltaX > 0 && deltaY < 0) {
    uhol = atan (deltaX / deltaY) + 1.5708;
  }

  // modra bodka
  else if ((bodB.x < bodA.x) && (bodB.y == bodA.y)) { 
    uhol = 3.14159; // 180 stupnov
  }
  else if (deltaX > 0 && deltaY > 0) {
    uhol = atan (deltaY / deltaX) + 3.14159;
  }

  // cierna bodka
  else if ((bodB.x == bodA.x) && (bodB.y < bodA.y)) { 
    uhol = 4.71239; // 270 stupnov
  }
  else if (deltaX < 0 && deltaY > 0) {
    uhol = atan (deltaX / deltaY) + 4.71239;
  }
